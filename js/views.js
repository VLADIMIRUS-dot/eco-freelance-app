...
const View = {
    ...
    renderProfile(profileData, isAdmin) {
        ...
        // Авто-цвет, если не задан (хотя Store должен это гарантировать)
        ...
        return `
            ...
            <div class="geo-section">
                ...
                <div id="bubbles-cloud" class="bubbles-container">
                    ${this._generateBubblesHTML(geo)}
                </div>
            </div>
            
            <div class="action-buttons">
                <button class="btn btn-primary" data-action="contact-telegram">
                    <i class="fa-brands fa-telegram"></i> Написать мне
                </button>
                <button class="btn btn-primary btn-attention" data-action="nav-to-calc">
                    <i class="fa-solid fa-calculator"></i> Заказать разработку
                </button>
                <button class="btn btn-outline" data-action="download-resume">
    ...
    _generateBubblesHTML(regions) {
        if (!regions) return '';
        // Перемешиваем и берем топ-10 для отображения
        const shuffled = [...regions].sort(() => Math.random() - 0.5).slice(0, 12);
        return shuffled.map(name => `<div class="bubble">${name}</div>`).join('');
    },

    /**
     * Запуск анимации пузырей (вызывать после вставки HTML)
     * Оптимизировано: использует requestAnimationFrame вместо setInterval
     */
    initBubblesAnimation(containerId) {
        ...
        const animateRandomBubble = () => {
            // Если вкладка не активна, не анимируем (экономия батареи)
            if (document.hidden || !container.offsetParent) {
                setTimeout(animateRandomBubble, 1000);
                return;
            }
            ...
            // Случайная задержка между 300мс и 800мс
    ...
    renderProjectsList(projects) {
        ...
        return projects.map(project => {
            let progressColor = '#2ecc71'; // Green
            if (project.status === 'paused') progressColor = '#f1c40f'; // Yellow
            else if (project.status === 'analysis') progressColor = '#3498db'; // Blue

            // Сериализуем объект в строку JSON для передачи в data-атрибут (для модалки)
            // Внимание: для больших объектов лучше использовать ID и искать в Store,
            // но для мелких можно так (с escape). Здесь используем ID для надежности.
            ...
            if (param.type === 'number') {
                inputHTML = `
                    <div class="input-with-suffix">
                        <input type="number" class="calc-input styled-num-input" 
                               data-id="${param.id}" 
                               value="${param.default}" 
                               min="0" step="1">
                        <span class="input-suffix">${param.suffix || ''}</span>
                    </div>`;
            ...
            const servicesHTML = obj.services.map((srv, srvIndex) => {
                ...
                <div class="service-row">
                    <div class="custom-select-wrapper">
                        <select data-action="update-est-service" data-obj-idx="${objIndex}" data-srv-idx="${srvIndex}">
                            ${options}
                        </select>
                        <i class="fa-solid fa-chevron-down select-arrow"></i>
                    </div>
                    <div style="display:flex; align-items:center; gap:10px;">
                        <span style="white-space:nowrap;">${srv.price.toLocaleString()} ₽</span>
                        <i class="fa-solid fa-trash text-red" data-action="remove-est-service" data-obj-idx="${objIndex}" data-srv-idx="${srvIndex}"></i>
                    </div>
                </div>`;
            }).join('');
            ...
            return `
                <div class="estimate-object-card">
                    <div class="obj-header">
                        <input type="text" class="obj-name-input" value="${obj.name}" data-action="update-est-name" data-obj-idx="${objIndex}">
                        <span class="remove-obj-btn" data-action="remove-est-obj" data-obj-idx="${objIndex}">Удалить</span>
                    </div>
                    <div class="obj-params-row">
                        <label><i class="fa-solid fa-industry"></i> Источников:</label>
                        <input type="number" class="obj-sources-input" value="${obj.sourcesCount}" data-action="update-est-sources" data-obj-idx="${objIndex}">
                    </div>
                    <div class="obj-services-list">
                        ${servicesHTML.length ? servicesHTML : '<p style="font-size:0.8rem; color:#999; text-align:center; padding:10px;">Добавьте услуги</p>'}
                    </div>
                    <div style="text-align:right; margin-bottom:10px; margin-top:10px; border-top:1px solid #eee; padding-top:5px;">
                        <small>Итого: <strong>${objTotal.toLocaleString()} ₽</strong></small>
                    </div>
                    <button class="btn btn-outline full-width" style="font-size:0.8rem; padding:8px;" data-action="add-est-service" data-obj-idx="${objIndex}">
    ...
    renderCRM(partners, filterStatus, totalDebt, totalPotential) {
        ...
        const statsHTML = `
            <div class="crm-search-box">
                ...
                <input type="text" placeholder="Поиск..." class="full-width" data-action="crm-search">
            </div>
            <div class="crm-stats-row">
                <div class="stat-box">
                    <strong class="text-debt">${totalDebt.toLocaleString()} ₽</strong>
                    <small>Долг мне</small>
                </div>
                <div class="stat-box">
            ...
            <div class="crm-filters">
                <span class="crm-chip ${filterStatus === 'all' ? 'active' : ''}" data-action="crm-filter" data-val="all">Все</span>
                <span class="crm-chip alert ${filterStatus === 'debt' ? 'active' : ''}" data-action="crm-filter" data-val="debt">⚠️ Должники</span>
                <span class="crm-chip ${filterStatus === 'active' ? 'active' : ''}" data-action="crm-filter" data-val="active">В работе</span>
                <span class="crm-chip ${filterStatus === 'lead' ? 'active' : ''}" data-action="crm-filter" data-val="lead">Лиды</span>
            </div>
        `;
        ...
        if (partners.length === 0) {
            listHTML = `<div style="text-align:center; padding:30px; color:#999;">Никого не найдено</div>`;
        } else {
                ...
                <div class="crm-card-modern">
                    ...
                    <div class="crm-details" id="crm-details-${p.id}">
                        ...
                        <div class="crm-btns-row">
                            <div class="crm-action-btn" data-action="crm-open-tg" data-username="${p.username}">
                                <i class="fa-brands fa-telegram" style="color:#2481cc"></i> TG
                            </div>
                            <div class="crm-action-btn" data-action="crm-copy-inn" data-inn="${p.inn}">
                                <i class="fa-solid fa-copy"></i> ИНН
                            </div>
                            <div class="crm-action-btn" style="color:var(--status-red)" data-action="crm-delete" data-id="${p.id}">
